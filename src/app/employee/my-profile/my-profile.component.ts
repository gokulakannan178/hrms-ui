...
export class MyProfileComponent implements OnInit {
  ...
  certUrl1:any;
  profileimg:string;
  //  urlLink : string = "assets/img/profiles/avatar-02.jpg"
  documentpolcyid:any
  ...
  constructor(public apiService: ApiService, 
    ...
    public sanitizer:DomSanitizer,  private spinner: NgxSpinnerService,
    private pagerService: PagerService,) { }

  ngOnInit(): void {
    
    this.userObj=JSON.parse(sessionStorage.getItem("userObj")),
    //  this.empid=JSON.parse(sessionStorage.getItem("empid")),
  ...
  profilepic(event){
      ...
      this.spinner.show();
      this.apiService.postFile('/common/docupload/profileimage', event.target.files[0]).subscribe(
        data => {
          this.spinner.hide();
          this.certUrl1 = data.response.data.uri;
          console.log("uploaded file data", data);
          console.log("uploaded file", this.certUrl1);

          var body={
            profileImg:this.certUrl1,
           
          }
          this.apiService.putMethod6("/employee/updateprofileimage" ,this.emplyoeeid,body).subscribe( data=>{
            this.viewprofileimg(this.certUrl1)
            
//send event to header
this.dataSharingService.profilepic.next(this.certUrl1)

          // this.apiService.putMethod6('/employee',body).subscribe( data => {
  ...
  removepicture() {
    ...
    this.apiService.putMethod6("/employee/updateprofileimage" ,this.emplyoeeid,body).subscribe(
      data => {
        alert("Deleted Successfully");
        this.getsingle();
        this.profileimg = ''
        this.dataSharingService.profilepic.next('')
      }
    )
  }
...
singlebankinfo(id)
{
  this.apiService.getMethod5("/bankInformation/employee",id).subscribe(
    data => {
...
saveCertImage(event,data2,uri,index) {
  ...
  this.apiService.postFile('/common/docupload/documents', event.target.files[0]).subscribe(
    data => {
      ...
      let body={
        employeeID:this.userObj.userName,
...
public downloadExcel(fileuri) {
  ...
  getsingle(){
    ...
    // this.emplyoeeid=this.userObj.uniqueId,
    // this.apiService.getMethod2("/employee" ,this.emplyoeeid).subscribe(
      ...
      data => {
        ...
        // data?.ref?.employee?.name?data?.ref?.employee?.name:"NA"
        ...
        console.log("this.emplyoee?.emergencyContact?.primary?.phoneNumber[0]",this.emplyoee?.emergencyContact?.primary?.phoneNumber);        
        this.emergencycontact.get("primaryphone").setValue((this.emplyoee?.emergencyContact?.primary?.phoneNumber)?(this.emplyoee?.emergencyContact?.primary?.phoneNumber[0]):"NA")
        this.emergencycontact.get("primaryphone1").setValue((this.emplyoee.emergencyContact.primary.phoneNumber)?(this.emplyoee?.emergencyContact?.primary?.phoneNumber[1]):"NA")
        ...
        this.emergencycontact.get("secondaryphone").setValue((this.emplyoee.emergencyContact.secondary.phoneNumber)?(this.emplyoee?.emergencyContact?.secondary?.phoneNumber[0]):"NA")
        this.emergencycontact.get("secondaryphone1").setValue((this.emplyoee.emergencyContact.secondary.phoneNumber)?(this.emplyoee?.emergencyContact?.secondary?.phoneNumber[1]):"NA")
        if (this.emplyoee.profileImg == '' || this.emplyoee.profileImg == null){
          if (this.emplyoee.gender=='male') {
            // this.profileimg = '../../../assets/img/profile_male.png';
            this.profileimg = './../../../assets/img/profile_male.png'
            // this.certUrl = '../../../assets/img/profile_male.png'
            console.log("male", this.profileimg)
          }
          else if (this.emplyoee.gender=='female') {
            this.profileimg = './../../../assets/img/profile_female.png'
            // this.profileimg = '../../../assets/img/profile_female.png';
            // this.certUrl = '../../../assets/img/profile_female.png';
            console.log("female", this.profileimg)
          }
        }else{
          this.profileimg=this.apiUrl+this.emplyoee.profileImg

        }
        // this.viewprofileimg(this.emplyoee.profileImg)
      },
      err => {
        // this.spinner.hide();
      }
