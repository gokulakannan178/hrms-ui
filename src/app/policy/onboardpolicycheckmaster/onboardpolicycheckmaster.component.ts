...
@Component({
  selector: 'app-onboardpolicycheckmaster',
  templateUrl: './onboardpolicycheckmaster.component.html',
  styleUrls: ['./onboardpolicycheckmaster.component.css']
})
export class OnboardpolicycheckmasterComponent implements OnInit {
  ...
  public getAllActiveCheckMaster(search, pageno) {
    ...
    this.apiService.postMethod("/onboardingchecklistmaster/filter?pageno=" + pageno + "&limit=" + this.showItemActive, body).subscribe(data => {
      this.spinner.hide();
      this.activeCheckMaster = data.response.data.OnboardingCheckListMaster;
  ...
  public getAllDisabledCheckMaster(search, pageno) {
    ...
    this.apiService.postMethod("/onboardingchecklistmaster/filter?pageno=" + pageno + "&limit=" + this.showItemDisabled, body).subscribe(data => {
      this.spinner.hide();
      this.disabledCheckMaster = data.response.data.OnboardingCheckListMaster;
  ...
  sumbitstatus(){
    ...
    if (this.typedetials == 'enable' || this.typedetials == "disable") {
      this.spinner.show();
      this.apiService.putMethod("/onboardingchecklistmaster/status/" + this.typedetials, this.empid).subscribe(
        data => {
  ...
  delete(){
    ...
    if (this.typedetials == 'delete') {
        this.spinner.show();
        this.apiService.deleteMethod1('/onboardingchecklistmaster/status/delete', this.empid).subscribe(
          data => {
  ...
  useraction(type,id){
    if(type == 'view'){
      sessionStorage.setItem("OnBoardCheckMasterViewId",id)
      this.router.navigate(['/policy/onboardpolicycheckmasterview'])
     }
     else if(type == 'edit'){
      sessionStorage.setItem("OnBoardCheckMasterEditId",id)
      this.router.navigate(['/policy/onboardpolicycheckmasteredit'])
     }
