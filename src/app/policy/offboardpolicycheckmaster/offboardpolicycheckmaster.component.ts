...
@Component({
  selector: 'app-offboardpolicycheckmaster',
  templateUrl: './offboardpolicycheckmaster.component.html',
  styleUrls: ['./offboardpolicycheckmaster.component.css']
})
export class OffboardpolicycheckmasterComponent implements OnInit {
  ...
  public getAllActiveOffboardPolicy(search, pageno) {
    ...
    this.apiService.postMethod("/offboardingchecklistmaster/filter?pageno=" + pageno + "&limit=" + this.showItemActive, body).subscribe(data => {
      this.spinner.hide();
      this.activeOffboardPolicy = data.response.data.OffboardingCheckListMaster;
  ...
  public getAllDisabledOffboardPolicy(search, pageno) {
    ...
    this.apiService.postMethod("/offboardingchecklistmaster/filter?pageno=" + pageno + "&limit=" + this.showItemDisabled, body).subscribe(data => {
      this.spinner.hide();
      this.disabledOffboardPolicy = data.response.data.OffboardingCheckListMaster;
  ...
  sumbitstatus(){
    ...
    if (this.typedetials == 'enable' || this.typedetials == "disable") {
      this.spinner.show();
      this.apiService.putMethod("/offboardingchecklistmaster/status/" + this.typedetials, this.empid).subscribe(
        data => {
  ...
  delete(){
    ...
    if (this.typedetials == 'delete') {
        this.spinner.show();
        this.apiService.deleteMethod1('/offboardingchecklistmaster/status/delete', this.empid).subscribe(
          data => {
  ...
  useraction(type,id){
    if(type == 'view'){
      sessionStorage.setItem("offboardPolicyMasterViewId",id)
      this.router.navigate(['/policy/offboardcheckmasterview'])
     }
     else if(type == 'edit'){
      sessionStorage.setItem("offboardPolicyMasterEditId",id)
      this.router.navigate(['/policy/offboardcheckmasteredit'])
     }
